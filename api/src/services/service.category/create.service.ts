...
export const createCategory = async (name: string, description: string, metaID: number, userID: number) => {
    ...
    const isCategoryExist = await pc.servicecategory.findFirst({ where: { name: name } });

    if (isCategoryExist)
        throw new HTTPException(HttpStatusCode.BAD_REQUEST, { message: 'service_category_already_exist_please_change_the_category_name' })
